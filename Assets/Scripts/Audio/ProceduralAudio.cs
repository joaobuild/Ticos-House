...
namespace TicosHouse
{
    ...
    public sealed class ProceduralAudio : MonoBehaviour
    {
        ...
        {
            int rate=22050, count=(int)(length*rate); float[] data=new float[count];
            for(int i=0;i<count;i++) {
                ...
                data[i]=Mathf.Clamp((wave*(1-noise)+(float)(rng.NextDouble()*2-1)*noise)*env*.5f,-.95f,.95f);
            }
            AudioClip clip=AudioClip.Create(name,count,1,rate,false); clip.SetData(data,0); clips[name]=clip;
        }
        void Update() { foreach(var s in fps) s.volume=Online&&!Quiet?FpsVolume*.72f:0; }
        ...
        {
            if(!Online||Quiet)return;
            AudioSource s=Free(fps); s.transform.position=Listener!=null?Listener.position:Vector3.zero;
            s.panStereo=Mathf.Clamp(relative.x/10,-.9f,.9f); s.volume=FpsVolume*.72f;
        ...
        public void House(HouseCue cue,float position)
        {
            ...
            switch(cue) {
                ...
                case HouseCue.Growl:clip="growl";break; case HouseCue.Scream:clip="scream";break;
            ...
            s.volume=cue==HouseCue.Snore?.78f:cue==HouseCue.Heart?.22f:.75f;
            s.PlayOneShot(clips[clip]);
        }
        public void Scare()
        {
            foreach(var a in fps)a.Stop(); AudioSource s=Free(house); s.spatialBlend=0; s.volume=.85f; s.PlayOneShot(clips["scare"]);
        }
